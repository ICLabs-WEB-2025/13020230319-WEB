...
@section('content')
    ...
    <div class="sidebar">
        ...
        <a href="{{ route('sim.create') }}" class="active"><i class="fas fa-plus-circle me-2"></i>Tambah SIM</a>
        <a href="{{ route('chat.index') }}"><i class="fas fa-comments me-2"></i>Chat</a>
    </div>

    <div class="content">
        <h2 class="animate__animated animate__fadeIn text-shadow"><i class="fas fa-plus-circle me-2"></i> Tambah SIM</h2>
        @if (session('error'))
        ...
        <div class="card shadow-lg border-0 card-hover animate__animated animate__fadeInUp">
            <div class="card-body p-4">
                <form action="{{ route('sim.store') }}" method="POST" id="createSimForm">
                    @csrf
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <input type="text" class="form-control @error('nomor_ktp') is-invalid @enderror" id="nomor_ktp" name="nomor_ktp" value="{{ old('nomor_ktp') }}" required>
                            @error('nomor_ktp')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            <select class="form-select @error('jenis_sim') is-invalid @enderror" id="jenis_sim" name="jenis_sim" required>
                                <option value="" disabled {{ old('jenis_sim') ? '' : 'selected' }}>Pilih Jenis SIM</option>
                                <option value="A" {{ old('jenis_sim') == 'A' ? 'selected' : '' }}>A</option>
                                <option value="B" {{ old('jenis_sim') == 'B' ? 'selected' : '' }}>B</option>
                                <option value="C" {{ old('jenis_sim') == 'C' ? 'selected' : '' }}>C</option>
                                <option value="D" {{ old('jenis_sim') == 'D' ? 'selected' : '' }}>D</option>
                            </select>
                            @error('jenis_sim')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                    ...
                    <button type="submit" class="btn btn-primary w-100"><i class="fas fa-save me-2"></i> Simpan SIM</button>
                </form>
            </div>
        </div>
    </div>
